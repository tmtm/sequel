...
リテラル文字列を生成するさらに簡単な方法として、{core_extensions extension}[link:files/doc/core_extensions_rdoc.html] を使用することもできます。これは <tt>String#lit</tt> メソッドを追加し、Ruby 言語に Sequel の DSL を統合する他のメソッドを追加します:
...
識別子を生成する他の方法は、Sequel の {virtual row support}[link:files/doc/virtual_rows_rdoc.html] を使用することです:
...
各 SQL 節に影響するメソッドにどんなものがあるかを見るには、{"Dataset Basics" guide}[link:files/doc/dataset_basics_rdoc.html] を見てください。
